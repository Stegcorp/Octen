...
for (const user of users) {
    ...
    let divеTxt = document.createElement('div')
    let button = document.createElement('button')
    divеTxt.innerText = `name -`+user.name + `  age - `+user.age +` status - `+user.status
    button.innerText ='додати до улюблених'
    document.body.appendChild(divan)
    divan.append(divеTxt,button)
button.onclick = function (e) {
    let text = divеTxt.innerText
    favorites.push(user)
